...
- 复核员可从**批次详情**发起「校样核对放行」：逐条选择本批次采用的校样编号，后端校验每份校样都已被接收（accepted）且 `relatedCode` 指向本批次；任一份遗漏或来源不一致时批次与放行记录都不会写入，并在响应与详情中指出出问题的具体校样。校验通过后在同一事务内把批次推进到 `released` 并生成可经 `relatedCode` 追溯到该批次的放行决定（`POST /api/runs/:id/release`，预检 `GET /api/runs/:id/release-check`）。直接调用批次迁移接口放行会被拒绝，原有独立放行入口 `/api/release` 保持不变。
