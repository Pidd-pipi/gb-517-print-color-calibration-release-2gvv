...
func New(cfg config.Config, db *gorm.DB, redisClient *redis.Client, logger *slog.Logger) *gin.Engine {
	...
	runReleaseRepository := repository.NewRunReleaseRepository(db)
	...
	runReleaseService := service.NewRunReleaseService(printRunRepository, colorProofRepository, runReleaseRepository)
	...
	runReleaseHandler := handler.NewRunReleaseHandler(runReleaseService)
	...
	runReleaseHandler.Register(api)
